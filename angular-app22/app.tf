...
resource "aws_key_pair" "app_key2" { 
  key_name   = "app_key2" 
...
resource "aws_instance" "app_instance_3" { 
  ...
  key_name               = aws_key_pair.app_key2.key_name
  associate_public_ip_address = true
  tags = { 
    Name = "app_server1" 
  }

  user_data = templatefile("${path.module}/userdata.sh", {ELB_EP=var.alb_dns_name})
}

resource "aws_instance" "app_instance_4" { 
  ami                    = "ami-0e86e20dae9224db8" 
  instance_type          = "t2.micro"
  subnet_id              = aws_subnet.app_public_subnet_2.id
  vpc_security_group_ids = [aws_security_group.app_web_sg.id]
...
resource "aws_lb_target_group_attachment" "app_attachment_3" { 
  target_group_arn = aws_lb_target_group.app_tg2.arn
  target_id        = aws_instance.app_instance_3.id
  port             = 3000
}

resource "aws_lb_target_group_attachment" "app_attachment_4" { 
  target_group_arn = aws_lb_target_group.app_tg2.arn
  target_id        = aws_instance.app_instance_4.id
  port             = 3000
}

# Outputs
output "alb_dns_name" {
  value = aws_lb.app_lb2.dns_name 
}
...
output "ec2_instance_ips" {
  value = [aws_instance.app_instance_3.public_ip, aws_instance.app_instance_4.public_ip] 
}
